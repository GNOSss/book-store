...
export const useBooksInfinite = () => {
  ...
  const getBooks = ({ pageParam }: { pageParam: number }) => {
    ...
    // fetchBooks({ category_id, newBook, limit, currentPage });
    return fetchBooks({ category_id, newBook, limit, currentPage });
  };

  const { data, fetchNextPage, hasNextPage, isFetching } = useInfiniteQuery(
    ['books', location.search],
    ({ pageParam = 1 }) => getBooks({ pageParam }), // getBooks의 반환값을 바로 반환
    {
  ...
  return { books, pagination, isEmpty, isBooksLoading: isFetching, fetchNextPage, hasNextPage };
};
